...
connection = pymysql.connect(
    host='localhost',
    user='root',
    password='Chinnu#61012',  # Make sure this matches what you entered in the command line
    database='SRA'  # Replace with your actual database name
)
...
def insert_data(name, email, res_score, timestamp, no_of_pages, reco_field, cand_level, skills, recommended_skills, courses):
    ...
    insert_sql = f'''
        INSERT INTO {DB_table_name} (name, email, resume_score, timestamp, total_pages, predicted_field, user_level, actual_skills, recommended_skills, recommended_course)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
    '''
    try:
        cursor.execute(insert_sql, (name, email, res_score, timestamp, no_of_pages, reco_field, cand_level, skills, recommended_skills, courses))
        connection.commit()
...
if st.button('Login'):
    if ad_user == 'machine_learning_hub' and ad_password == 'mlhub123':
        st.success("Welcome Jaya Kishore")

        # Display Data
        cursor.execute('''SELECT * FROM user_data''')  # Added space after SELECT
        data = cursor.fetchall()

        st.header("**User's👨‍💻 Data**")
        df = pd.DataFrame(data, columns=[
            'ID', 'Name', 'Email', 'Resume Score', 
            'Timestamp', 'Total Page', 'Predicted Field', 
        ...
        # Admin Side Data
        query = 'SELECT * FROM user_data;'  # Added space after SELECT
        ...
        fig = px.pie(values=level_values, names=level_labels, title="Pie-Chart📈 for User's👨‍💻 Experienced Level")
        st.plotly_chart(fig)
